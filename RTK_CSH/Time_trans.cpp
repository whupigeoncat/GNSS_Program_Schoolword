...
bool GPSTimeToMJDTime(const GPSTIME* GT, MJDTIME* MJD)
{
	...
	double days = GT->SecOfWeek / 86400.0;
	MJD->Days = 44244 + GT->Week * 7 + floor(days);
	MJD->FracDay = days - floor(days);
...
double GetDiffTime(const GPSTIME* GT2, const GPSTIME* GT1)
{
	...
	diffweek = GT2->Week - GT1->Week;
	diffsec = GT2->SecOfWeek - GT1->SecOfWeek;
	difftime= diffweek * 604800.0 + diffsec;
